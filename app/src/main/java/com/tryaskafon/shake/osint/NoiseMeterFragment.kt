...
/**
 * NoiseMeterFragment — показывает уровень шума в децибелах через микрофон.
...
class NoiseMeterFragment : Fragment() {
    ...
    private val readAmplitude = object : Runnable {
        override fun run() {
            ...
            try {
                ...
                if (amp > 0) {
                    // Формула: dB = 20 * log10(amplitude / 32768) + 90 (смещение для «воздушного» уровня)
                    val db = 20 * log10(amp.toDouble() / 32768.0) + 90.0
                    val clamped = db.coerceIn(30.0, 120.0)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        b.btnResetMax.setOnClickListener { maxDb = 0.0; b.tvMaxDb.text = "Макс: 0.0 дБ" }
    }

    private fun startMeter() {
        if (ContextCompat.checkSelfPermission(requireContext(), Manifest.permission.RECORD_AUDIO)
            != PackageManager.PERMISSION_GRANTED) {
            b.tvDbValue.text = "Нет доступа к микрофону"
            return
        }
        try {
            ...
            recorder!!.apply {
                ...
                setOutputFile("/dev/null")  // не пишем файл, только читаем амплитуду
                prepare()
                start()
            }
            running = true
    ...
    override fun onDestroyView() { stopMeter(); super.onDestroyView(); _b = null }
}
